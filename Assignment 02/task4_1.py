...
# ax.set_title(r"Eigenvalues $\lambda_1$ and $\lambda_2$ as function of $V_r$")
fig.tight_layout(pad=0)
fig.savefig("report/figs/detuning_4_1.pdf", bbox_inches="tight", pad=0)
...
for Vr in [-10, 0, 10]:
    ...
    l, = ax.plot(x, psi0squared, label=r"$\psi_1$, $\nu_r = %d$" % Vr)
    ax.plot(x, psi1squared, label=r"$\psi_2$, $\nu_r = %d$" % Vr, dashes=[4, 4], color=l.get_color())
ax.legend(fontsize="small", handlelength=1)
ax.set_ylabel(r"$|\Psi|^2$")
ax.set_xlabel(r"$x/L$")
ax.set_xlim([0, 1])
fig.tight_layout(pad=0)
fig.savefig("report/figs/detuning_localization.pdf", bbox_inches="tight", pad=0)
