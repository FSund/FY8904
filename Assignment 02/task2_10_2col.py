...
fig, axes = plt.subplots(3, 1, figsize=[3.45, 4], sharex=True)
for ax in axes[0:-1]:
...
ax.set_title(r"Re $\Psi(x,t)$", fontsize=titlefontsize)
...
ax.set_title(r"Im $\Psi(x,t)$", fontsize=titlefontsize)
# plt.xlabel(r"Time $t/(2mL^2/\hbar)$")
# plt.ylabel(r"Position $x/L$")
plt.colorbar(im, ax=ax)
# plt.tight_layout()
...
ax.set_title(r"$|\Psi(x,t)|^2$", fontsize=titlefontsize)
plt.colorbar(im, ax=ax)

axes[2].set_xlabel(r"Time $t/(2mL^2/\hbar)$")

for ax in axes:
    ax.set_ylabel(r"Position $x/L$")

for ax in axes:
    ax.xaxis.set_major_locator(MultipleLocator(0.2))

fig.tight_layout(pad=0)
fig.savefig("report_2col/figs/box_psi0.pdf", bbox_inches="tight", pad=0)
